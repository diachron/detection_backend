...
public class ChangesDetector {
     ...
     * @throws SQLException
     */
    public ChangesDetector(Properties prop, String changesOnt, String changesOntSchem, String assoc) throws ClassNotFoundException, IOException, RepositoryException, SQLException {
        initialize(prop);
        this.changesOntology = changesOnt;
        this.changesOntologySchema = changesOntSchem;
        this.associationsGraph = assoc;
    }

    public ChangesDetector(String propFile, String changesOnt, String changesOntSchem, String assoc) throws ClassNotFoundException, IOException, RepositoryException, SQLException {
        Properties prop = new Properties();
        InputStream inputStream = new FileInputStream(propFile);
        prop.load(inputStream);
        initialize(prop);
    ...
    public void detectAssociations(String oldVersion, String newVersion) {
...
//            System.out.println("Detecting " + simpleChange);
        jdbc.executeUpdateQuery("sparql " + prefixes + query, false);
//        consumeSCFromAssoc(prefixes);
    ...
    private void consumeSCFromAssoc(String prefixes) {
        ...
        query = "SELECT distinct ?sc WHERE { \n"
        ...
        for (String addedSc : sChanges) {
            if (addedSc.endsWith("Add_Type_To_Individual")) {
...
//                + "WHERE {\n"
//                + "GRAPH <" + changesOntologySchema + "> {\n"
//                + "?addSC co:opposite_with ?deleteSC.\n"
//                + "}"
//                + "GRAPH <" + changesOntology + "> {\n"
//                + "?assoc a co:Association;\n"
//                + "     co:assoc_p1 ?old;\n"
...
//        jdbc.executeUpdateQuery("sparql " + prefixes + query, false);
    ...
    public void detectSimpleChanges(String oldVersion, String newVersion, String[] simpleChanges) {
        ...
        for (String simpleChange : this.simpleChanges) {
            ...
            query = query.replaceAll("<v1>", "<" + oldVersion + ">");
            query = query.replaceAll("<v2>", "<" + newVersion + ">");
            query = query.replaceAll("<assoc>", "<" + associationsGraph + ">");
//            System.out.println("Detecting " + simpleChange);
